...
class GenerationEngine:
    def __init__(
        ...
        self.stop_sequences = {
            text: torch.tensor(
                tokenizer.encode(text, add_special_tokens=False).ids, dtype=torch.long
            )
    ...
    def add(self, input_ids: Tensor, max_new_tokens: int):
        ...
        if input_ids.size(1) + max_new_tokens > 1024:
            raise ValueError("prompt plus generated tokens exceeds KV-cache capacity")
    ...
    def _make_decode_batch(self, exclude: set[int] | None = None):
        ...
        active = [
            req
            for _, req in self.slots.active()
    ...
    def _accept_sampled_token(self, req: GenerateRequest, token: Tensor) -> bool:
        ...
        req.input_ids = torch.cat((req.input_ids, token.reshape(1, 1)), dim=1)
    ...
    def step(self):
        ...
        _, active, active_slots = self._make_decode_batch(exclude=just_prefilled)
        if active:
    ...
    def frm0():
        ...
        def engine_worker():
            while not worker_stop.is_set():
                try:
                    ...
                    root.after(0, lambda exc=exc: generation_failed(exc))
                    time.sleep(0.05)
        ...
        def generation_finished(req: GenerateRequest, generated_text: str):
            ...
            for request_id, text in generated_texts:
                response_text.insert(tkinter.END, f"[request {request_id}]\n{text}\n\n")
        ...
        submit_btn = ttk.Button(frm, text="Submit", command=callback_0)
        status_label = ttk.Label(frm, textvariable=status_var)

        response_text.configure(state="disabled")

        requests_text.grid(row=0, column=0, columnspan=2, sticky="nsew", pady=(0, 8))
        response_text.grid(row=1, column=0, columnspan=2, sticky="nsew", pady=(0, 8))
        input_text.grid(row=2, column=0, sticky="ew", padx=(0, 8))
        submit_btn.grid(row=2, column=1, sticky="ew")
...
def main() -> None:
    ...
    engine = GenerationEngine(model, tokenizer, max_batch=4, stop_strings=("<|im_end|>",))
