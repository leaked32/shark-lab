	...
	ApplicationState& app_state, uint32_t peer_id, uint32_t self_id)
{
	...
	payload_messages.emplace_back(json::object{{"role", "system"}, {"content", system_prompt}});
	...
	for (auto it = messages.rbegin(); it != messages.rend(); ++it) {
		size_t cost = shark::str::estimate_tokens(it->content) + 16;
	...
	for (const auto& m : kept) {
		payload_messages.emplace_back(json::object{
			{"role", m.sender_id == self_id ? "user" : "assistant"}, {"content", m.content}});
	}
