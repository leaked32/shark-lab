...
def make_sft_batch(
    ...
    while len(lx) < batch_count:
        if not valid_indices:
            raise RuntimeError("No valid SFT records remain while creating a batch.")
        ...
        try:
            cx, cy = dataset_context.to_sft_tensors(tokenizer, -1, cindex, seq_clip)
            ...
            if cx.size(0) != cy.size(0):
                raise ValueError(f"Input and target lengths differ: {cx.size(0)} != {cy.size(0)}")

            if cx.numel() == 0:
                raise ValueError("The converted sample is empty.")

        except Exception as exc:
            print(f"Removing failed SFT record index={cindex}: {type(exc).__name__}: {exc}")
            ...
            index_queue[:] = [index for index in index_queue if index != cindex]
...
def train(opt: shark.format.manifest_options):
    ...
    match dataset_type:
        case 0:
            ...
            if not os.path.exists(dataset_train):
                raise FileNotFoundError(f"Training dataset does not exist: {dataset_train}")
    ...
    for step in range(start_step, max_steps):
        ...
        match dataset_type:
            ...
            case 1:
                if dataset_context is None:
                    raise RuntimeError("SFT dataset was not initialized.")
            ...
            case _:
                raise RuntimeError(f"Unexpected dataset_type: {dataset_type}")
        ...
        if DEBUG_GRADIENTS:
            ...
            print(
                f"step {step}: gradient norm before clipping = {total_grad_norm.detach().item()}",
                flush=True,
            )
    ...
    if max_steps > start_step:
        ...
        print(f"No training performed: start_step={start_step}, max_steps={max_steps}.")
