...
enum class message_type { INFO, DEBUG, EXCEPTION };
...
class backend
{
  public:
	...
	void push(
		message_type type, std::string msg)
	{
		{
...
inline void submit(
	message_type type, const std::string& msg)
{
...
constexpr void info(
	const std::format_string<Types...>& Fmt, Types&&... Args)
{
	std::string info_str = std::format(Fmt, std::forward<Types>(Args)...);
	submit(message_type::INFO, info_str);

	// std::string info_str = std::format("INFO:  {}", std::format(Fmt,
	// std::forward<Types>(Args)...)); submit(info_str);
}
...
struct pinch : std::exception
{
	...
	// pinch(std::string_view func_name, std::string_view information) : std::exception{},
	// msg_(std::format("trace {}:{}", func_name, information)) { }
...
template <typename... Types>
void assert(
	bool condition, const std::format_string<Types...>& Fmt, Types&&... Args)
{
	if (!condition) {
	...
	Types&&... Args)
{
	...
	if (decltype(N)(result) != N) {
		log::debug("Invocation of function has failed: {} {:#x}", SHARK_FUNCTION,
				   decltype(N)(result));
...
struct profiler
{
	...
	profiler(
		size_t reserve, bool print_on_lap) : start(clock::now()), print_on_lap(print_on_lap)
	{
	...
	static void print_duration(
		duration value, std::optional<std::string_view> msg = std::nullopt)
	{
...
class tlm_node
{
  ...
  public:
	...
	{
		return doc != nullptr ? std::optional<std::string_view>{*doc}
	...
	std::optional<std::reference_wrapper<const std::map<std::string, tlm_node>>>
	c_get_children() const
	{
		if (children) {
			return *children;
		}
  ...
  private:
	// lit::print("{}  {}", sizeof (std::map<std::string, tlm_node>), sizeof
	// (std::unique_ptr<std::map<std::string, tlm_node>>));
...
template <typename T>
std::tuple<T, T> from_interval_text(
...
struct delayer
{
	...
	void shorter(
		std::chrono::system_clock::duration dura)
	{}
};
...
class strand
{
  public:
	...
	strand(
		io_context& parent, std::coroutine_handle<> coro) : parent_(parent)
	{
		addresses_.push(coro);
	}

	strand(
		io_context& parent, strand&& obj) noexcept :
...
struct awaitable
{
	...
	struct promise_type : promise_type_base
	{
		...
		{
			return awaitable{Handle_Type::from_promise(*this)};
		}
		...
		void unhandled_exception()
		{
			...
			try {
				...
				if (slice) {
					log::debug("slice: {}; current coroutine: {}; exception what: {}",
...
struct io_context
{
	...
	void erase_coroutine(
		const std::shared_ptr<strand>& target)
	{
		std::scoped_lock lock(mtx_context_);

		std::erase_if(context_, [&](auto& x) { return x == target; });
	}

	template <typename TAsyncAwaitable>
	void spawn(
		TAsyncAwaitable&& coro)
	{
		std::scoped_lock mtx_context_lock(mtx_context_);
	...
	void run()
	{
		...
		while (!empty()) {
			...
			// Because the delayer automatically delays for the timespan to the
...
struct deadline_timer
{
	...
	struct deadline_timer_condition : strand::Condition
	{
		...
		bool evaluate(
			delayer& local_delayer) override
		{
			return local_delayer.before(end_time_point_);
		}
		~deadline_timer_condition() override = default;
		deadline_timer_condition(
			std::chrono::system_clock::time_point end_time_point) : end_time_point_(end_time_point)
		{}
	};
		...
		std::chrono::system_clock::duration dura)
	{
		...
		// NOTE: use steady_clock to trade performance for precision
	}
};
...
class multi_executor
{
  ...
  private:
	shark::async::awaitable<int> async_load(
		int id)
	{
		...
		while (!shutting_down_) {
			...
			co_await timer.expires_after(std::chrono::milliseconds(100));
		}
		// log::debug("Executing the job 2");
		co_return 0;
	}
