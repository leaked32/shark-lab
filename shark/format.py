...
class TrainOption:
    ...
    def __post_init__(self) -> None:
        ...
        if self.dataset_type not in (0, 1):
            raise ValueError("train.dataset_type must be 0 (CORPUS) or 1 (SFT)")
        ...
        if self.optimizer_learning_rate <= 0.0:
            raise ValueError("train.optimizer_learning_rate must be > 0")
        if self.adamw_weight_decay < 0.0:
            raise ValueError("train.adamw_weight_decay must be >= 0")
        if not 0.0 <= self.adamw_beta1 < 1.0:
            raise ValueError("train.adamw_beta1 must satisfy 0 <= beta1 < 1")
        if not 0.0 <= self.adamw_beta2 < 1.0:
            raise ValueError("train.adamw_beta2 must satisfy 0 <= beta2 < 1")
...
class SystemOption:
    ...
    def __post_init__(self) -> None:
        ...
        if self.dtype not in allowed_dtypes:
            raise ValueError(
                f"system.dtype must be one of {sorted(allowed_dtypes)}, got {self.dtype!r}"
            )
...
def reject_unknown_fields(section_name: str, data: dict[str, Any], option_type: type) -> None:
    ...
    if unknown_fields:
        ...
        raise ValueError(f"Unknown field(s) in [{section_name}]: {names}")
...
def load_manifest_options(path: str | Path) -> manifest_options:
    ...
    if bias is not False:
        raise ValueError("model.bias=true is unsupported by the current model")
    ...
    train = TrainOption(
        ...
        optimizer_learning_rate=float(train_raw["optimizer_learning_rate"]),
        adamw_weight_decay=float(train_raw["adamw_weight_decay"]),
...
def format_chat(
    ...
    for message in messages:
        formatted.append(f"<|im_start|>{message['role']}\n{message['content']}<|im_end|>\n")
...
class JsonlDataset:
    ...
    def to_sft_tensors(
        self, tokenizer: Tokenizer, ignore_index: int, conversation_index: int, seq_clip: int | None
    ) -> tuple[Tensor, Tensor]:
        # Python masking
        token_ids: list[int] = []
        ...
        for message in conversation:
            ...
            if mask and not message.content.strip():
                raise ValueError(f"assistant message cannot be emptyconversation: {conversation}")

        if not any(train_mask):
            raise ValueError(
                f"conversation contains no assistant targetsconversation: {conversation}"
            )
        ...
        if seq_clip is not None:
            if seq_clip <= 0:
                raise ValueError(f"seq_clip must be positive or None, got {seq_clip}")
    ...
    def validate_sft_indices(self, tokenizer) -> tuple[list[int], list[int]]:
        ...
        for cindex in range(item_count):
            try:
                ...
                if cx.ndim != 1 or cy.ndim != 1:
                    raise ValueError(
                        f"Expected one-dimensional tensors, got x.ndim={cx.ndim}, y.ndim={cy.ndim}"
                    )

                if cx.size(0) != cy.size(0):
                    raise ValueError(
                        f"Input and target lengths differ: {cx.size(0)} != {cy.size(0)}"
                    )
                ...
                print(f"Skipping invalid SFT record index={cindex}: {type(exc).__name__}: {exc}")
        ...
        print(f"SFT validation complete: {len(valid_indices)} valid, {skipped_count} skipped.")
