...
class GPTOption:
    ...
    def __post_init__(self) -> None:
        ...
        if self.q_head % self.kv_head != 0:
            raise ValueError("model.q_head must be divisible by model.kv_head")
        if self.chan % self.q_head != 0:
            raise ValueError("model.chan must be divisible by model.q_head")
        if self.mlp_mul <= 0:
...
class IKVCache(Protocol):
    ...
    def in_forward(
        self, q: Tensor, k: Tensor, v: Tensor, acts: Tensor, rope: RoPE, active_slots: Tensor
...
class DecoderState:
    def __init__(self, kv_cache: list[IKVCache] | None = None, active_slots: Tensor | None = None):
...
class KVCache0:
    ...
    def in_forward(self, q: Tensor, k: Tensor, v: Tensor, acts: Tensor, rope: RoPE, _):
...
class KVCache1:
    ...
    def in_forward(
        ...
        for row in range(sz_batch):
            ...
            q[row : row + 1], k[row : row + 1] = RoPE.apply_rotary(
                q[row : row + 1], k[row : row + 1], cos, sin
            )
        ...
        if self.k is None or self.v is None:
            ...
            self.k = k.new_zeros((self.max_batch, kv_head, new_cap, head_dim))
            self.v = v.new_zeros((self.max_batch, kv_head, new_cap, head_dim))
        elif older_cap < least_cap:
            # Auto growth with pre-allocation
            new_cap = max(least_cap, older_cap * 2)
            extra = new_cap - older_cap
            self.k = torch.cat(
                (self.k, self.k.new_zeros((self.max_batch, kv_head, extra, head_dim))), dim=2
            )
            self.v = torch.cat(
                (self.v, self.v.new_zeros((self.max_batch, kv_head, extra, head_dim))), dim=2
            )
    ...
    def gather(self, active_slots: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        for row, slot_tensor in enumerate(active_slots):
            ...
            for physical_block in self.block_tables[slot]:
                ...
                k_dense[row, :, write_pos : write_pos + take, :] = self.k_pool[
                    physical_block, :, :take, :
                ]
                v_dense[row, :, write_pos : write_pos + take, :] = self.v_pool[
                    physical_block, :, :take, :
                ]
                write_pos += take
...
class Attention2(nn.Module):
    ...
    def __init__(self, opt: GPTOption, layer_id: int, sdpa: bool = True):
        ...
        self.register_buffer(
            "causal_mask_cache", torch.empty(0, 0, dtype=torch.bool), persistent=False
        )
    ...
    def get_causal_mask(self, start_pos: int, q_len: int, kv_len: int, device) -> Tensor:
        ...
        return self.causal_mask_cache[start_pos : start_pos + q_len, :kv_len].view(
            1, 1, q_len, kv_len
        )
    ...
    def forward(self, acts: Tensor, state: DecoderState) -> Tensor:
        ...
        if cache is None:
            ...
            q, k, v, causal_mask = cache.in_forward(q, k, v, acts, self.rope, active_slots)
...
class TransformerBlock(nn.Module):
    def __init__(self, norm: INorm, norm1: INorm, attn: IAttention, mlp: IFeedForward):
...
class GPT(nn.Module):
    ...
    def forward(
        self, input: Tensor, targets: Tensor | None = None, state: DecoderState | None = None
    ...
    def sample_next_token(
        logits: Tensor, temperature: float = 1.0, top_k: int | None = None
    ) -> Tensor:
